...
extern "C" {
    ...
    #[no_mangle]
    fn init_binary_node(op: libc::c_char, left: *mut node_t,
                        right: *mut node_t) -> *mut node_t;
    #[no_mangle]
    fn init_var_node(name: var_name_t) -> *mut node_t;
    #[no_mangle]
    fn init_sequence_node(statement_count: size_t,
                          statements: *mut *mut node_t) -> *mut node_t;
    #[no_mangle]
    fn init_print_node(expr: *mut node_t) -> *mut node_t;
    #[no_mangle]
    fn init_let_node(var: var_name_t, value: *mut node_t) -> *mut node_t;
    #[no_mangle]
    fn init_if_node(condition: *mut binary_node_t, if_branch: *mut node_t,
                    else_branch: *mut node_t) -> *mut node_t;
    #[no_mangle]
    fn init_while_node(condition: *mut binary_node_t, body: *mut node_t)
     -> *mut node_t;
    #[no_mangle]
...
pub type char_predicate_t
    =
    Option<unsafe extern "C" fn(_: libc::c_char) -> bool>;
...
pub unsafe extern "C" fn is_variable_name(mut c: libc::c_char) -> bool {
    return *(*__ctype_b_loc()).offset(c as libc::c_int as isize) as
               libc::c_int &
...
#[no_mangle]
pub unsafe extern "C" fn save_position(mut state: *mut parser_state_t)
 -> size_t {
    return ftell((*state).stream) as size_t;
}
#[no_mangle]
pub unsafe extern "C" fn restore_position(mut state: *mut parser_state_t,
                                          mut position: size_t) {
    fseek((*state).stream, position as libc::c_long, 0 as libc::c_int);
}
#[no_mangle]
pub unsafe extern "C" fn rewind_one(mut state: *mut parser_state_t) {
    fseek((*state).stream, -(1 as libc::c_int) as libc::c_long,
          1 as libc::c_int);
}
/*
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn advance(mut state: *mut parser_state_t)
 -> libc::c_char {
    loop  {
...
#[no_mangle]
pub unsafe extern "C" fn try_advance(mut state: *mut parser_state_t,
                                     mut predicate: char_predicate_t)
 -> libc::c_char {
    let mut next: libc::c_char = advance(state);
    if next as libc::c_int != '\u{0}' as i32 &&
           !predicate.expect("non-null function pointer")(next) {
        rewind_one(state);
        return '\u{0}' as i32 as libc::c_char
    }
    return next;
...
#[no_mangle]
pub unsafe extern "C" fn advance_until_separator(mut state:
                                                     *mut parser_state_t)
 -> *mut libc::c_char {
    let mut result: *mut libc::c_char =
        malloc(::std::mem::size_of::<[libc::c_char; 101]>() as libc::c_ulong)
            as *mut libc::c_char;
    if !result.is_null() {
    } else {
        __assert_fail(b"result != NULL\x00" as *const u8 as
                          *const libc::c_char,
                      b"src/parser.c\x00" as *const u8 as *const libc::c_char,
                      86 as libc::c_int as libc::c_uint,
                      (*::std::mem::transmute::<&[u8; 48],
                                                &[libc::c_char; 48]>(b"char *advance_until_separator(parser_state_t *)\x00")).as_ptr());
    }
...
pub unsafe extern "C" fn num(mut state: *mut parser_state_t) -> *mut node_t {
    ...
    if num_string.is_null() { return 0 as *mut node_t }
    let mut num_end: *mut libc::c_char = 0 as *mut libc::c_char;
    *__errno_location() = 0 as libc::c_int;
    let mut value: value_t =
        strtol(num_string, &mut num_end, 0 as libc::c_int);
    if *__errno_location() != 0 as libc::c_int ||
           *num_end as libc::c_int != '\u{0}' as i32 {
...
#[no_mangle]
pub unsafe extern "C" fn factor(mut state: *mut parser_state_t)
 -> *mut node_t {
    if try_advance(state,
                   Some(is_open_paren as
                            unsafe extern "C" fn(_: libc::c_char) -> bool)) !=
           0 {
        let mut node: *mut node_t = expression(state);
        if try_advance(state,
                       Some(is_close_paren as
    ...
    let mut var: libc::c_char =
        try_advance(state,
                    Some(is_variable_name as
                             unsafe extern "C" fn(_: libc::c_char) -> bool));
    if var != 0 { return init_var_node(var) }
    return num(state);
}
#[no_mangle]
pub unsafe extern "C" fn term(mut state: *mut parser_state_t) -> *mut node_t {
    ...
    loop  {
        let mut next: libc::c_char =
            try_advance(state,
                        Some(is_factor_op as
                                 unsafe extern "C" fn(_: libc::c_char)
                                     -> bool));
        if next == 0 { break ; }
...
#[no_mangle]
pub unsafe extern "C" fn expression(mut state: *mut parser_state_t)
 -> *mut node_t {
...
#[no_mangle]
pub unsafe extern "C" fn comparison(mut state: *mut parser_state_t)
 -> *mut binary_node_t {
    let mut left: *mut node_t = expression(state);
    let mut op: libc::c_char =
        try_advance(state,
                    Some(is_comparison_op as
                             unsafe extern "C" fn(_: libc::c_char) -> bool));
    return init_binary_node(op, left, expression(state)) as
               *mut binary_node_t;
}
#[no_mangle]
pub unsafe extern "C" fn statement(mut state: *mut parser_state_t,
                                   mut end: *mut bool) -> *mut node_t {
    while try_advance(state,
                      Some(is_comment_start as
                               unsafe extern "C" fn(_: libc::c_char) -> bool))
              != 0 {
        skip_line(state);
    }
    ...
    if strcmp(next, b"ELSE\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
    ...
    if strcmp(next, b"END\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
        free(next as *mut libc::c_void);
        next = advance_until_separator(state);
        if next.is_null() ||
               !(strcmp(next, b"IF\x00" as *const u8 as *const libc::c_char)
                     == 0 as libc::c_int ||
                     strcmp(next,
    ...
    *end = 0 as libc::c_int != 0;
    if strcmp(next, b"PRINT\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
        free(next as *mut libc::c_void);
        return init_print_node(expression(state))
    }
    if strcmp(next, b"LET\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
        free(next as *mut libc::c_void);
    ...
    if strcmp(next, b"IF\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
        free(next as *mut libc::c_void);
        ...
        if !next.is_null() &&
               strcmp(next, b"ELSE\x00" as *const u8 as *const libc::c_char)
                   == 0 as libc::c_int {
            free(next as *mut libc::c_void);
            else_branch = sequence(state);
            next = advance_until_separator(state)
        } else { else_branch = 0 as *mut node_t }
        if next.is_null() ||
               strcmp(next, b"END\x00" as *const u8 as *const libc::c_char) !=
                   0 as libc::c_int {
            free(next as *mut libc::c_void);
    ...
    if strcmp(next, b"WHILE\x00" as *const u8 as *const libc::c_char) ==
           0 as libc::c_int {
        free(next as *mut libc::c_void);
...
#[no_mangle]
pub unsafe extern "C" fn sequence(mut state: *mut parser_state_t)
 -> *mut node_t {
    ...
    loop  {
        ...
        if statement_count == statement_capacity {
            statement_capacity =
                if statement_capacity > 0 as libc::c_int as libc::c_ulong {
                    statement_capacity.wrapping_mul(2 as libc::c_int as
                                                        libc::c_ulong)
                } else { 1 as libc::c_int as libc::c_ulong };
            statements =
                realloc(statements as *mut libc::c_void,
                        (::std::mem::size_of::<*mut node_t>() *
                             statement_capacity as usize) as libc::c_ulong) as
                    *mut *mut node_t;
            if !statements.is_null() {
            } else {
                __assert_fail(b"statements != NULL\x00" as *const u8 as
                                  *const libc::c_char,
                              b"src/parser.c\x00" as *const u8 as
                                  *const libc::c_char,
                              344 as libc::c_int as libc::c_uint,
                              (*::std::mem::transmute::<&[u8; 35],
                                                        &[libc::c_char; 35]>(b"node_t *sequence(parser_state_t *)\x00")).as_ptr());
            }
        }
    ...
    if statement_count > 0 as libc::c_int as libc::c_ulong {
        statements =
            realloc(statements as *mut libc::c_void,
                    (::std::mem::size_of::<*mut node_t>() *
                         statement_count as usize) as libc::c_ulong) as
                *mut *mut node_t;
        if !statements.is_null() {
        } else {
            __assert_fail(b"statements != NULL\x00" as *const u8 as
...
pub unsafe extern "C" fn parse(mut stream: *mut FILE) -> *mut node_t {
    let mut state: parser_state_t =
        { let mut init = parser_state_t{stream: stream,}; init };
    let mut ast: *mut node_t = sequence(&mut state);
    if !at_end(&mut state) { free_ast(ast); return 0 as *mut node_t }
    return ast;
}
